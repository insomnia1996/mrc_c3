...
from keyselect import feature_to_label
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        ...
        tokens_b = tokenizer.tokenize(example.text_b)#choice

        tokens_c = tokenizer.tokenize(example.text_c)#question
...
class c3Processor(DataProcessor):
    def __init__(self, data_dir):
        ...
        for sid in range(3):
            ...
            for subtask in ["d", "m"]:
                if sid==2:
                    with open(os.path.join(self.data_dir , ["train.json", "dev.json", "test.json"][sid]),
                              "r", encoding="utf8") as f:
                        data += json.load(f)
                        break#test只load一次
                else:
                    with open(os.path.join(self.data_dir , subtask + "-" + ["train.json", "dev.json", "test.json"][sid]),
                              "r", encoding="utf8") as f:
                        data += json.load(f)
            if sid == 0:
                random.shuffle(data)
                ...
                for j in range(len(data[i][1])):
                    try:
                        ...
                        if sid<2:
                            ...
                            d += [-1]
                        self.D[sid] += [d]#equal to append(d)
                    except:
                        pass

    def get_train_examples(self):
        """See base class."""
        print("train examples: ", len(self.D[0]))
        return self._create_examples(self.D[0], "train")

    def get_test_examples(self):
        """See base class."""
        print("test examples: ", len(self.D[2]))
        return self._create_examples(self.D[2], "test")

    def get_dev_examples(self):
        """See base class."""
        print("dev examples: ", len(self.D[1]))
    ...
    def _create_examples(self, data, set_type):
        ...
        if os.path.exists(cache_dir):
            ...
            for (i, d) in enumerate(data):
                ...
                for k in range(4):
                    if data[i][6]==-1:#test, no answer
                        answer = str(0)#随便给一个答案，不影响test
                    elif data[i][2 + k] == data[i][6]:
                        answer = str(k)
...
def main():
    ...
    model = SequenceLabeling(emb_size= 256,
                             pos_emb_size= 128,
                             hidden_size= 1024,
                             enc_layers= 8,
                             dropout= 0.1,
    ...
    if args.do_eval:
        ...
        feature_dir = os.path.join(args.data_dir, 'dev_features{}_key.pkl'.format(args.max_seq_length))
    ...
    if args.do_train:
        ...
        feature_dir = os.path.join(args.data_dir, 'train_features{}_key.pkl'.format(args.max_seq_length))
        if os.path.exists(feature_dir):
        ...
        for ie in range(int(args.num_train_epochs)):
            ...
            with tqdm(total=int(steps_per_epoch), desc='Epoch %d' % (ie + 1)) as pbar:
                for step, batch in enumerate(train_dataloader):
                    ...
                    input_ids, input_mask, segment_ids, label_ids = batch#这里还是(batch_size*4,seq_len),到dataparallel后被分发到GPU上了
                    flat_input_ids = input_ids.view(-1, input_ids.size(-1))
                    flat_attention_mask = input_mask.view(-1, input_mask.size(-1))
                    # input shape: (batch_size * 4, seq_len)
                    # logger.info(flat_input_ids.shape)#[4,512]
                    lpos = torch.arange(input_ids.size(-1), dtype=torch.long, device=input_ids.device).unsqueeze(
                        0).repeat(flat_input_ids.size(0), 1)
                    rpos = torch.arange(input_ids.size(-1) - 1, -1, -1, dtype=torch.long,
                                        device=input_ids.device).unsqueeze(0).repeat(flat_input_ids.size(0), 1)

                    output = model(flat_input_ids, flat_attention_mask, lpos, rpos)
                    loss = output['loss']
                    new_attention_mask = torch.argmax(output['logits'], dim=-1)#(batch_size * 4, seq_len)#print(loss.shape, logits.shape)#[batch_size],[batch_size,num_classes]
            ...
            if args.do_eval:
                ...
                for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader):
                    ...
                    for i in range(len(logits)):
                        logits_all += [logits[i]]
                    #print("logits and labels",logits_all,label_ids)
                ...
                if 1:#eval_accuracy >= best_accuracy:
                    torch.save(model.state_dict(), os.path.join(args.output_dir, "key_model_best.pt"))
                    best_accuracy = eval_accuracy

        model.load_state_dict(torch.load(os.path.join(args.output_dir, "key_model_best.pt")))
        torch.save(model.state_dict(), os.path.join(args.output_dir, "key_model.pt"))

    model.load_state_dict(torch.load(os.path.join(args.output_dir, "key_model.pt")))
            ...
            for i in range(len(logits_all)):
                for j in range(len(logits_all[i])):
                    ...
                    if j == len(logits_all[i]) - 1:
                        ...
                        f.write(" ")
    #TODO: test不能用label标记，要改sequence_labeling的predict
    #注意： test的tgt是随便标的，但是不影响模型效果，因为传入的tgt只用来计算loss，没有backward运算。
    if args.do_test:
        test_examples = processor.get_test_examples()
        feature_dir = os.path.join(args.data_dir, 'test_features{}_key.pkl'.format(args.max_seq_length))
        if os.path.exists(feature_dir):
        ...
        for f in test_features:
            ...
            for i in range(n_class):
                ...
                input_mask[-1].append(f[i].input_mask)
                segment_ids[-1].append(f[i].segment_ids)
            label_id.append(f[0].label_id)#test_label全是0
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(test_dataloader):
            ...
            with torch.no_grad():
                tmp_test_loss, logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids)
