...
from pytorch_modeling import BertConfig, BertForMultipleChoice, ALBertConfig, ALBertForMultipleChoice, MyModelForMultipleChoice
from modeling_nezha import NeZhaForMultipleChoice, NeZhaConfig
...
def main():
    ...
    if 'albert' in args.bert_config_file:
        if 'google' in args.bert_config_file:
            ...
            bert_config = ALBertConfig.from_json_file(args.bert_config_file)
            model = ALBertForMultipleChoice(bert_config, num_choices=n_class)
    elif 'nezha' in args.bert_config_file:
        bert_config = NeZhaConfig.from_json_file(args.bert_config_file)
        model = NeZhaForMultipleChoice(bert_config, num_choices=n_class)
    else:
        bert_config = BertConfig.from_json_file(args.bert_config_file)
        #model = BertForMultipleChoice(bert_config, num_choices=n_class)
        model = MyModelForMultipleChoice(bert_config, num_choices=n_class)
        ...
        for ie in range(int(args.num_train_epochs)):
            ...
            with tqdm(total=int(steps_per_epoch), desc='Epoch %d' % (ie + 1)) as pbar:
                for step, batch in enumerate(train_dataloader):
                    ...
                    loss, logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids)
                    #print(loss.shape, logits.shape)#[batch_size],[batch_size,num_classes]
                    #if len(loss)==1 and type(loss)==tuple:
                    #    loss = loss[0]
                    if n_gpu > 1:
