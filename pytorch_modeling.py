...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # to the first token.
        #print("hidden state before pooler: ",hidden_states.shape)#[batch*num_class,seq_len,hidden]
        first_token_tensor = hidden_states[:, 0]#"CLS"标签对应hidden_state
        pooled_output = self.dense(first_token_tensor)
        pooled_output = self.activation(pooled_output)
        return pooled_output#[batch*num_class,hidden]
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        sequence_output = encoded_layers[-1]#最后一层，(batch*num_class,seq_len,hidden_size)
        pooled_output = self.pooler(sequence_output)#(batch*num_class,hidden_size)
        if not output_all_encoded_layers:
...
class BertForMultipleChoice(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, return_logits=True):
        flat_input_ids = input_ids.view(-1, input_ids.size(-1))
        flat_token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1))
        flat_attention_mask = attention_mask.view(-1, attention_mask.size(-1))
        _, pooled_output = self.bert(flat_input_ids, flat_token_type_ids, flat_attention_mask,
                                     output_all_encoded_layers=False)#[batch*num_class,hidden]
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        reshaped_logits = logits.view(-1, self.num_choices)#[batch,num_class]
...
#Modified to suit C3 task
class KeyContentSelection(nn.Module):
    def __init__(self, config):
        super(KeyContentSelection, self).__init__()
...
class MyModelForMultipleChoice(PreTrainedBertModel):

    def __init__(self, config, num_choices=2):
        super(MyModelForMultipleChoice, self).__init__(config)
        self.num_choices = num_choices
        self.content_selection = SequenceLabeling(emb_size= 256,
                                                 pos_emb_size= 128,
                                                 hidden_size= 1024,
                                                 enc_layers= 8,
                                                 dropout= 0.1,
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, return_logits=True):
        ...
        lpos = torch.arange(input_ids.size(-1),dtype=torch.long, device=input_ids.device).unsqueeze(0).repeat(flat_input_ids.size(0),1)
        rpos = torch.arange(input_ids.size(-1)-1,-1,-1,dtype=torch.long, device=input_ids.device).unsqueeze(0).repeat(flat_input_ids.size(0), 1)
        #logger.info("position shape: %s, %s."%(lpos.shape,rpos.shape))#[4,512]

        #output = self.content_selection(flat_input_ids, flat_attention_mask, lpos, rpos)
        #key_loss = output['loss']
        #new_attention_mask = torch.argmax(output['logits'], dim=-1)#(batch_size * 4, seq_len)
        new_attention_mask = flat_attention_mask
        _, pooled_output = self.bert(flat_input_ids, flat_token_type_ids, new_attention_mask,
                                     output_all_encoded_layers=False)#[batch*num_class,hidden]
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        reshaped_logits = logits.view(-1, self.num_choices)#[batch,num_class]
        ...
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            #logger.info("key loss: "+str(key_loss.item())+" ; LM loss: "+str(loss_fct(reshaped_logits, labels).item()))
            #loss = 0.2 * key_loss + 0.8 * loss_fct(reshaped_logits, labels)
            loss = loss_fct(reshaped_logits, labels)
            if return_logits:
                return loss, reshaped_logits
            else:
